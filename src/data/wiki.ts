 ...
 * 중이라, 쓰다가 막히는 쪽보다 넉넉히 열어 두는 쪽을 골랐다.
 *
 * 상한을 정하는 것은 저장소가 아니라 **편집 차이 계산**이었다. LCS 표가 토큰 수의
 * 곱이라 본문이 길면 제곱으로 커진다. 그런데 `wikiDiff`가 앞뒤 공통을 먼저 떼어 내고
 * `MAX_CELLS`로 표 크기를 따로 막고 있어, 길이를 올려도 메모리가 늘지 않는다 —
 * 넘어가는 편집은 통째 재작성으로 보고 검토로 보낸다. 재어 보니 50,000자 본문에서
 * 키 입력 하나당 2ms로, 60fps 예산(16ms) 안쪽이다.
 *
 * 이 값을 더 올릴 때도 같은 것을 본다. 느려지는 것은 길이 자체가 아니라 **한 번에
 * 갈아엎는 덩어리의 크기**다.
 */
